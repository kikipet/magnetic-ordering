...
def run_model(run_name, magfile, datafile):
    ### Setup
    # if we already have a .pt file to pull data from
    data = torch.load(magfile)
    formula_list_mp, sites_list, id_list = pickle.load(open(datafile, 'rb'))
    ...
    ### prepare data
    index_tr, index_va, index_te, dataloader, dataloader_valid = create_dataloaders(data, batch_size=20)
